...
def computeDurationOfScan(scanNumber, t=None, vis=None, returnSubscanTimes=False,
                          verbose=False, gapFactor=None, includeDate=False, mymsmd=None):
    """
    ...
    -- Todd Hunter
    """
    if (t==None and vis == None and (mymsmd==None or mymsmd=='')):
    ...
    if (t == None):
        if (casadef.casa_version < casaVersionWithMSMD):
            ...
            return
        if (mymsmd == None or mymsmd == ''):
            mymsmd = createCasaTool(msmdtool)
            mymsmd.open(vis)
        else:
            keepmymsmd = True
    elif (mymsmd == None or mymsmd==''):  # fix for CSV-2903 on 29-Apr-2016
        mymsmd = createCasaTool(msmdtool)
        mymsmd.open(vis)
    else:
        keepmymsmd = True
    if (t == None):
        t = pickTimesForScan(mymsmd, scanNumber)
    else:
        t = np.unique(t)
    if (len(t) <= 1):
        if (casadef.casa_version < casaVersionWithMSMD):
            ...
            if (scanNumber==1):
                duration = np.min(pickTimesForScan(mymsmd, scanNumber+1)) - t[0]
            else:
                duration = t[0] - np.max(pickTimesForScan(mymsmd, scanNumber-1))
        ...
        if (mymsmd != None and mymsmd != '' and not keepmymsmd):
            mymsmd.close()
        d = np.max(t) - np.min(t)
        ...
        for i in range(1,len(t)):
            ...
            if (tdiff > gapFactor*avgInterval):
                ...
                startTime = t[i]
            previousTime = t[i]
        if (droppedTimeTotal > 0 or verbose):
            print "+++++ Scan %d: total dropped time = %.1f seconds = %.1f minutes" % (scanNumber,droppedTimeTotal, droppedTimeTotal/60.)
        if daygaps > 0:
            print "              large time gaps: %.1f sec = %.1f hrs = %.1f days" % (daygaps,daygaps/3600.,daygaps/86400.)
...
def createCasaTool(mytool):
    ...
    return(myt)
#------------------------------------------------------------------------
def pickTimesForScan(mymsmd, scan):
    """
...
def radecOffsetToRadec(radec, rao, deco, replaceDecDotsWithColons=True,
    ...
    return(rad2radec(newrarad,newdecrad,replaceDecDotsWithColons=replaceDecDotsWithColons,prec=prec,verbose=verbose,delimiter=delimiter))
#------------------------------------------------------------------------
...
def sol_sqld_mean_2(myms, mymsmd, scan_id, spw_id, sub_scan_id, ant_names,avss=False):
    ...
    if not (isinstance(scan_id, np.int32) or  isinstance(scan_id, int)):
    ...
    if len(sub_scan_id) == 1:
        ...
        timeran = []
        avsub = []
        for tr in sub_inf[3].iteritems():
            if tr[0] in sub_scan_id:
                timeran.append(tr[1])
                avsub.append(tr[0])
...
def sol_gentats_2(vis, orgtbl, Tant, skyScan, sciScan, spw, tgtSubScan,mymsmd,atmScan):
        ...
        for j in range(0, len(spw)):
            ...
            try:
                ...
                for pol in range(0, tsyst.shape[0]):
                    ...
                    Sun_Tant = Tant[antNames[i]][pols[pol]][j]
    ...
    return modtbl
#--------------------------
def sol_ampcal_2(vis, orgtbl, exisTbl, outCSV):
...
#                print apptr
                applycal(vis = vis, scan = sciScan[i], spw = '5,7,9,11',
#                         gaintable = modtbl, timerange = subInf[3][j],
                         gaintable = modtbl, timerange = apptr,
...
#                    print "write CSV \n"
    ...
    if exisTbl == True:
        ...
        for i in range(0, len(sciScan)):
            ...
            for j in range(3, subNum-1):

                print '### Tsys+Tant Cal of Scan:' + format(sciScan[i],'03d') +' SubScan:'+format(j,'03d')+' ###'
                modtbl = orgtbl+'tant_'+format(sciScan[i],'03d')+'_'+format(j, '03d')

                applycal(vis = vis, scan = sciScan[i], spw = '5,7,9,11', gaintable = modtbl,
                         timerange = subInf[3][j],interp = 'linear,linear', calwt = True, flagbackup = False)
